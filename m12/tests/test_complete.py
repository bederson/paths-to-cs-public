...
class TestComplete(unittest.TestCase):
    def test_complete(self):
        ...
        expected = urllib2.urlopen("http://"+bot_name+".appspot.com/respond?msg=complete%20"+item_id).read()
        ...
        response = urllib2.urlopen("http://"+bot_name+".appspot.com/respond?msg="+msg).read()
        item_id = re.findall(r'your todo list at item ([0-9]+)\.', response)[0]
        submitted = urllib2.urlopen("http://"+bot_name+".appspot.com/respond?msg=complete%20"+item_id).read()
        submitted = re.sub(r'[0-9]',r'...',submitted)

        self.assertEqual(expected, submitted, "\nWhen a new todo item is completed...\n[Expected response]:\t"+ expected +"...\n[Submitted response]:\t"+submitted)
