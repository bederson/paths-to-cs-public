...
class TestWorld(unittest.TestCase):
    def get_water_grid_str(self, world):
        ...
        for row in world.grid:
            line = ""
            for cell in row:
                line += str(cell.water_level)+"("+str(cell.get_elevation())+")"+ " "
            s += line + "\n"
        return s
    ...
    def test_flow_water_once_flat(self):
        ...
        expected_water = [[2.94, 0.03], [0.03, 0.0]]
        initial_msg = "\nInitial water levels(Elevation):\n" + self.get_water_grid_str(world) + "\n"
        world.flow_water()
    ...
    def test_flow_water_twice_flat(self):
        ...
        expected_water = [[2.8236, 0.0879], [0.0879, 0.0006]]
        initial_msg = "\nBefore 2nd round, water levels(Elevation):\n" + self.get_water_grid_str(world) + "\n"
        world.flow_water()
        world.apply_water_delta()
        valid, msg = self.is_valid_water(world, expected_water, initial_msg)
        self.assertTrue(valid, msg)
    ...
    def test_flow_water_once_hilly(self):
        world = World(world_dim=2, num_foods=0, num_water_sources=0)
        cell = WaterSourceCell(world, (0, 0), 3, 0)
        ...
        expected_water = [[2.9, 0.05], [0.05, 0.0]]
        initial_msg = "\nInitial water levels(Elevation):\n" + self.get_water_grid_str(world) + "\n"
        world.flow_water()
        world.apply_water_delta()
        valid, msg = self.is_valid_water(world, expected_water, initial_msg)
    ...
    def test_flow_water_twice_hilly(self):
        ...
        expected_water = [[2.706, 0.1365], [0.1365, 0.021]]
        initial_msg = "\nBefore 2nd round, water levels(Elevation):\n" + self.get_water_grid_str(world) + "\n"
        world.flow_water()
        world.apply_water_delta()
        valid, msg = self.is_valid_water(world, expected_water, initial_msg)
